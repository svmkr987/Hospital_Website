...
export default function Home() {
  ...
  const checkIsOpen = () => {
    const now = new Date();
    const utcOptions = { timeZone: 'Asia/Kolkata', hour12: false };
    const istDate = new Date(now.toLocaleString('en-US', utcOptions));
    ...
    if (day === 0) {
      return timeInMinutes >= 10 * 60 && timeInMinutes < 16 * 60;
    } else {
      return timeInMinutes >= 8 * 60 + 15 && timeInMinutes < 21 * 60;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-mint font-sans selection:bg-green selection:text-white overflow-x-hidden">
          ...
          <div className="flex items-center gap-4">
            {isAdminLoggedIn ? (
              <div className="flex items-center gap-3">
                <Link to="/admin" className="hover:text-sage transition-colors font-medium">Dashboard</Link>
                <span className="text-slate/50">|</span>
                <button onClick={handleAdminLogout} className="text-red-400 hover:text-red-300 transition-colors font-medium">Log Out</button>
              </div>
            ) : (
              <Link to="/admin" className="hover:text-sage transition-colors font-medium">Admin Login</Link>
            )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            <div className="hidden md:flex gap-8">
              ...
              <Link to="/contact" className="text-slate hover:text-green text-sm font-medium transition-colors">Enquiry</Link>
            </div>

            <div className="flex items-center gap-4">
              <Link
                to="/book"
                className="hidden sm:flex items-center gap-2 px-5 py-2.5 bg-mint text-slate hover:bg-sage/30 rounded-full text-sm font-medium transition-colors border border-sage/50"
              >
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-10 items-center">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Link 
                  to="/book"
                  className="inline-flex justify-center items-center gap-3 bg-green text-white px-6 py-4 rounded-2xl shadow-sm hover:bg-green/90 transition-colors"
                >
      ...
      <section id="services" className="py-24 bg-white relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            <motion.div whileHover={{ y: -5 }} className="bg-mint rounded-[40px] p-8 border border-sage hover:shadow-lg hover:shadow-sage/20 transition-all group">
              ...
              <p className="text-slate/80 leading-relaxed text-sm">Treatment for common ailments, fevers, infections, and routine medical checkups for all ages.</p>
            </motion.div>
            ...
            <motion.div whileHover={{ y: -5 }} className="bg-mint rounded-[40px] p-8 border border-sage hover:shadow-lg hover:shadow-sage/20 transition-all group">
              ...
              <p className="text-slate/80 leading-relaxed text-sm font-medium">Specialized care for Type 1 and Type 2 diabetes management, diet counseling, and complication prevention.</p>
            </motion.div>
            ...
            <motion.div whileHover={{ y: -5 }} className="bg-mint rounded-[40px] p-8 border border-sage hover:shadow-lg hover:shadow-sage/20 transition-all group">
              ...
              <p className="text-slate/80 leading-relaxed text-sm">In-house laboratory for quick and accurate blood tests, sugar monitoring, and essential diagnostics.</p>
            </motion.div>
            ...
            <motion.div whileHover={{ y: -5 }} className="bg-mint rounded-[40px] p-8 border border-sage hover:shadow-lg hover:shadow-sage/20 transition-all group">
              ...
              <p className="text-slate/80 leading-relaxed text-sm">Fully stocked pharmacy ensuring you get your prescribed medicines immediately after consultation.</p>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-mint border-t border-sage/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 max-w-2xl mx-auto">
            <h2 className="text-3xl font-display font-bold text-slate mb-4">Patient Testimonials</h2>
            <div className="w-20 h-1.5 bg-green rounded-full mx-auto mb-6"></div>
            <p className="text-slate/80 text-lg">
              Hear what our patients have to say about their experience and care at Shubham Clinic.
            </p>
          </div>
            ...
            {[
              {
                name: 'Rahul M.',
                role: 'Patient',
                content: 'Dr. Samskar is very patient and listens to all problems carefully. His diagnosis is accurate and treatment is very effective. Highly recommend him for diabetes management.',
                rating: 5
              },
              {
                name: 'Priya S.',
                role: 'Patient',
                content: 'Visited for a severe viral fever. The doctor explained everything clearly and did not prescribe unnecessary medicines. The in-house pharmacy is a big plus!',
                rating: 5
              },
              {
                name: 'Anil K.',
                role: 'Patient',
                content: 'Excellent clinic with a very clean environment. The staff is polite, and Dr. Samskar’s approach is very professional and comforting. Truly a family doctor.',
                rating: 5
              }
            ].map((testimonial, i) => (
              <div key={i} className="bg-white rounded-[40px] p-8 shadow-sm border border-sage relative">
                <Quote className="absolute top-8 right-8 w-8 h-8 text-sage/30" />
                <div className="flex gap-1 mb-6 text-green">
                  {[...Array(testimonial.rating)].map((_, i) => (
                    <Star key={i} className="w-4 h-4 fill-current" />
                  ))}
                </div>
                <p className="text-slate/80 leading-relaxed mb-8 italic">"{testimonial.content}"</p>
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-mint rounded-full flex items-center justify-center font-bold text-green border border-sage/50">
                    {testimonial.name.charAt(0)}
                  </div>
                  <div>
                    <h4 className="font-bold text-slate text-sm">{testimonial.name}</h4>
                    <span className="text-xs text-slate/50 font-medium uppercase tracking-wider">{testimonial.role}</span>
                  </div>
                </div>
              </div>
            ))}
